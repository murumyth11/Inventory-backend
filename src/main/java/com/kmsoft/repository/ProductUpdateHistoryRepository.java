...
public interface ProductUpdateHistoryRepository extends JpaRepository<ProductUpdateHistory, Integer>{
	...
	@Query(value="select * from product_update_history where product_fk=:id and (batch like :title or update_date like :title or update_data like :title  or updatefrom like :title or update_by like :title)order by  update_history_id desc",
			countQuery =  "SELECT count(*) from product_update_history where product_fk=:id",nativeQuery=true)
