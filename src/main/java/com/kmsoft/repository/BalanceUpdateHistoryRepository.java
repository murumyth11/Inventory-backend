...
public interface BalanceUpdateHistoryRepository extends JpaRepository<BalanceUpdateHistory, Integer> {
	...
	@Query(value="SELECT bh.balance_update_date,bh.cash_in,bh.updated_by,h.customer,h.phone,h.invoice,bh.balance,h.total from balance_update_history bh "
		...
		@Query(value="SELECT  bh.paymentmethod,bh.balance_update_date,bh.cash_in,bh.updated_by,c.customer_name as 'customer',h.phone,h.invoice ,bh.balance,h.total"
				+ " from balance_update_history bh join (header_bill h join customer c on h.customer_fk=c.customer_id) on bh.headerbill_fk=h.header_bill_id where bh.balance_update_date like :title or "
				+ "bh.cash_in like :title or c.customer_name like :title or h.phone like :title or h.invoice like :title  or bh.updated_by like "
				+ " :title "
