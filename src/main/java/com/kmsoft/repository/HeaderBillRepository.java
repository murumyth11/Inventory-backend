...
public interface HeaderBillRepository extends JpaRepository<HeaderBill, Integer> {
	...
	@Query(value = "SELECT * FROM Header_bill h join customer c on h.customer_fk=c.customer_id  WHERE h.date >= :startDate AND h.date <= :endDate  and (total like :title or status like  :title or c.customer_name like :title or invoice like :title or c.customer_phone like :title or date like :title ) "
			, nativeQuery = true)
	...
	List<Map<String, Object>> getbillbydate(@Param("startDate") Date startDate, @Param("endDate") Date endDate);
	 ...
	 @Query(value="SELECT * FROM header_bill h join customer c on h.customer_fk=c.customer_id WHERE h.isdraft=0 and ( c.customer_name LIKE :title"+" OR h.invoice LIKE :title "
	 +" OR h.subtotal LIKE :title "
			 +" OR h.total LIKE :title "
	 +" OR c.customer_phone LIKE :title  or h.status like  :title  or h.balance like :title or h.billed_by like :title or h.date like :title )ORDER BY header_bill_id DESC"
				 ,nativeQuery = true)
